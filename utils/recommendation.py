...
try:
    svd_data    = np.load(os.path.join(PROJECT_DIR, 'reports/svd_predictions.npy'))
    pmf_data    = np.load(os.path.join(PROJECT_DIR, 'reports/pmf_predictions.npy'))
    user_index  = joblib.load(os.path.join(PROJECT_DIR, 'processed/user_index.pkl'))
    movie_index = joblib.load(os.path.join(PROJECT_DIR, 'processed/movie_index.pkl'))
    movies_pd   = pd.read_csv(os.path.join(PROJECT_DIR, 'data/movies.dat'), sep='::', encoding='latin-1', engine='python', names=["MovieID","Title","Genres"])
...
def generate_recommendations(user_id, model, top_n=10):
    ...
    top_movies_ids = [ movies_indexs[i] for i in top_movies_indexs].copy()
...
def plot_user_comparison(user_id, reports_dir=REPORTS_DIR):
...
def plot_top_recommendations(user_ids=[1,2,3,4,5], reports_dir=REPORTS_DIR):
    all_recs = []
    for uid in user_ids:
